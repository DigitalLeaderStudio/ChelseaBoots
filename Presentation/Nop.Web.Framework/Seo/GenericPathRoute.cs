...
namespace Nop.Web.Framework.Seo
{
	...
	public partial class GenericPathRoute : LocalizedRoute
	{
		...
		public override RouteData GetRouteData(HttpContextBase httpContext)
		{
			...
			if (data != null && DataSettingsHelper.DatabaseIsInstalled())
			{
				...
				switch (urlRecord.EntityName.ToLowerInvariant())
				{
					case "product":
						{
							SetupRouteData(data, "Product", "ProductDetails", urlRecord.Slug, "productid", urlRecord.EntityId);
						}
						break;
					case "category":
						{
							SetupRouteData(data, "Catalog", "Category", urlRecord.Slug, "categoryid", urlRecord.EntityId);
						}
						break;
					case "manufacturer":
						{
							SetupRouteData(data, "Catalog", "Manufacturer", urlRecord.Slug,
								"manufacturerid", urlRecord.EntityId);
						}
						break;
					case "vendor":
						{
							SetupRouteData(data, "Catalog", "Vendor", urlRecord.Slug, "vendorid", urlRecord.EntityId);
						}
						break;
					case "newsitem":
						{
							SetupRouteData(data, "News", "NewsItem", urlRecord.Slug, "newsItemId", urlRecord.EntityId);
						}
						break;
					case "blogpost":
						{
							SetupRouteData(data, "Blog", "BlogPost", urlRecord.Slug, "blogPostId", urlRecord.EntityId);
						}
						break;
					case "topic":
						{
							SetupRouteData(data, "Topic", "TopicDetails", urlRecord.Slug, "topicId", urlRecord.EntityId);
						}
						break;
					default:
						{
		...
		void SetupRouteData(
			...
			object paramValue)
		{
			targetRoutData.Values["controller"] = controller;
			targetRoutData.Values["action"] = action;
			targetRoutData.Values["SeName"] = seName;
			targetRoutData.Values[paramName] = paramValue;

			// The hack for abitlity to redefine contoller from other namespace
			foreach (System.Web.Routing.Route route in RouteTable.Routes)
			{
				if (route.Defaults != null)
				{
					if (route.Defaults["controller"].ToString().Equals(controller)
						&& route.Defaults["action"].ToString().Equals(action))
					{
						targetRoutData.DataTokens["Namespaces"] = route.DataTokens["Namespaces"];
					}
				}
			}
		}
